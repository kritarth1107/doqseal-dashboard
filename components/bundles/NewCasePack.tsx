...
const REQUIREMENT_LABEL: Record<CatalogDocument["requirement"], string> = {
  required: "Required",
  conditional: "Depends on case",
  optional: "Optional",
};
...
export function NewCasePack() {
  ...
  const documents: CatalogDocument[] = useMemo(() => {
    const fromTemplate = toCatalogDocuments(template?.draft?.documentTypes);
    return fromTemplate.length ? fromTemplate : (selected?.documents ?? []);
  }, [template, selected]);
  ...
  const canSubmit = Boolean(template) && hasIdentity && !submitting;
            ...
            <div className="flex flex-col lg:flex-row gap-6 items-start">
              <section className="flex-1 min-w-0 w-full" aria-label="Templates" aria-busy={phase === "loading"}>
                {phase === "loading" ? (
                  ...
                  <ul
                    className={`grid gap-4 grid-cols-1 sm:grid-cols-2 ${selected ? "xl:grid-cols-2 2xl:grid-cols-3" : "lg:grid-cols-3"}`}
                  >
                    {visible.map((entry) => (
              ...
              {selected && (
                <SetupPanel onClose={close} title={selected.name}>
                  <PanelHeading entry={selected} onClose={close} />
                  {preparing ? (
                    ...
                    >
                            ...
                            <ul className="divide-y divide-gray-100 dark:divide-zinc-800">
                              {documents.map((d) => (
                                <li key={d.key} className="flex items-center justify-between gap-3 py-2 text-sm">
                                  ...
                                  <RequirementTag requirement={d.requirement} />
                                </li>
                              ))}
                            </ul>
                          </div>
                      ...
                      <div className="sticky bottom-0 px-5 py-4 border-t border-gray-100 dark:border-zinc-800 bg-white/95 dark:bg-[#111827]/95 backdrop-blur space-y-2">
                        {stepError && (
...
function SetupPanel({ title, onClose, children }: { title: string; onClose: () => void; children: ReactNode }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end lg:sticky lg:inset-auto lg:top-6 lg:z-auto lg:block lg:w-[400px] lg:shrink-0">
      <button
        type="button"
        aria-label="Close"
        tabIndex={-1}
      ...
      <div
        ...
        className="relative flex h-full w-full max-w-md flex-col overflow-y-auto bg-white dark:bg-[#111827] shadow-xl outline-none lg:h-auto lg:max-h-[calc(100vh-7rem)] lg:max-w-none lg:rounded-2xl lg:border lg:border-gray-200 lg:dark:border-zinc-800 lg:shadow-sm"
      >
        {children}
      </div>
    </div>
...
function PanelHeading({ entry, onClose }: { entry: CatalogEntry; onClose: () => void }) {
  const look = lookFor(entry.category);
  const Icon = look.icon;
  return (
    <div className="flex items-start gap-3 px-5 pt-5 pb-4 border-b border-gray-100 dark:border-zinc-800">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">{entry.categoryLabel}</p>
        <h2 className="font-semibold text-slate-900 dark:text-slate-100 leading-snug">{entry.name}</h2>
        <p className="text-xs text-slate-500 mt-0.5">{documentSummary(entry)}</p>
      </div>
      <button
        type="button"
        onClick={onClose}
        aria-label="Close template setup"
        className="p-1.5 -mr-1.5 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-zinc-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2563eb]"
      >
        <X className="w-4 h-4" />
      </button>
    </div>
...
function RequirementTag({ requirement }: { requirement: CatalogDocument["requirement"] }) {
  const tone =
  ...
  return (
    <span className={`shrink-0 text-[11px] font-medium px-2 py-0.5 rounded-full ${tone}`}>
